...
Examples:
  python build_vca_wide_unified_v2.py --sql vca_raw_extract.sql --out-wide vca_wide_unified.csv
  python build_vca_wide_unified_v2.py --in-csv raw_vca_extract.csv --out-wide vca_wide_unified.csv \
...
CAT_SUFFIXES = [
    "gf", "hs", "wh", "mill", "shop", "store", 
    "trader", "roaster", "exporter", "extractor", "other"
]
...
# =============================================================================
# ENVIRONMENT AND DATABASE HELPERS
# =============================================================================
...
def parse_json_or_list(s: Any) -> Optional[Any]:
    """Parse JSON string or list literal."""
...
# =============================================================================
# DATA TRANSFORMATION - BUILDING LONG FORMAT ROWS
# =============================================================================

def build_category_rows(w: pd.Series) -> List[Dict[str, Any]]:
    """Build rows for Q13 business categories."""
    rows = []
...
def build_per_category_rows(w: pd.Series, base_field: str, qkey: str, 
                           label_categories: bool) -> List[Dict[str, Any]]:
    """Build rows for per-category fields (business name, address, etc)."""
    rows = []
    meta = {k: w.get(k) for k in META_COLS if k in w}
...
def build_array_rows(w: pd.Series, col: str, qkey: str, category: Optional[str], 
                    label_categories: bool) -> List[Dict[str, Any]]:
    """Build rows for array/list fields."""
    rows = []
    meta = {k: w.get(k) for k in META_COLS if k in w}
...
def build_scalar_row(w: pd.Series, col: str, qkey: str, 
                    category: Optional[str] = None,
                    source: str = "responses") -> Dict[str, Any]:
    """Build a single row for scalar fields."""
    meta = {k: w.get(k) for k in META_COLS if k in w}
...
def build_gps_rows(w: pd.Series) -> List[Dict[str, Any]]:
    """Build rows for GPS coordinates."""
    rows = []
    meta = {k: w.get(k) for k in META_COLS if k in w}
    ...
    if isinstance(gps, dict):
        rows.append({
            **meta, 
            "question_key": "q28_vca_gps_latitude", 
            "category": None,
            "value": as_str(gps.get("latitude")), 
            "source": "responses", 
            "original_field": "q28_vca_gps_json"
        })
        rows.append({
            **meta, 
            "question_key": "q28_vca_gps_longitude", 
            "category": None,
            "value": as_str(gps.get("longitude")), 
            "source": "responses", 
            "original_field": "q28_vca_gps_json"
        })
    else:
        rows.append({
            **meta, 
            "question_key": "q28_vca_gps_raw", 
            "category": None,
            "value": as_str(w.get("q28_vca_gps_json")), 
            "source": "responses",
            "original_field": "q28_vca_gps_json"
        })
...
def run_dqc_for_row(long_row: Dict[str, Any], wide_row: pd.Series, 
                   selected_cats: set) -> Dict[str, Any]:
    """Run data quality checks for a single row."""
    ...
    if q in {"q22_type_of_coffee", "q22_type_of_coffee_all"} and dq_present:
        if val not in CHOICES_Q22_TYPE:
            dq_valid_choice = False
            reasons.append("invalid_choice")
    
    if q in {"q23_coffee_form", "q23_coffee_form_all"} and dq_present:
        if val not in CHOICES_Q23_FORM:
            dq_valid_choice = False
            reasons.append("invalid_choice")
    
    if q in {"q26_receive_coffee_from", "q26_receive_coffee_from_all"} and dq_present:
        if val not in CHOICES_Q26_FROM:
            dq_valid_choice = False
            reasons.append("invalid_choice")
    ...
    if q in {"q20_hullers_operated", "q18_max_operating_capacity", 
             "q19_max_storage", "q25_annual_kgs_received"}:
        num = clean_number(val)
    ...
    if q in {"q15_business_name", "q16_business_address", 
             "q18_max_operating_capacity", "q19_max_storage"} and cat:
    ...
    # Check Q14: Other business category validation
    if q == "q14_other_business_category":
        # Check if "Other" was selected in Q13
        q13_categories = []
        # Get Q13 business categories from the wide row
        q13_raw = as_str(wide_row.get("q13_business_category", ""))
        if q13_raw:
            # Split by " | " to get individual categories
            q13_categories = [cat.strip() for cat in q13_raw.split(" | ") if cat.strip()]
        
        other_selected = "Other" in q13_categories
        
        if other_selected:
            # If Other is selected, Q14 should not be empty
            if not dq_present:
                dq_dependency_ok = False
                reasons.append("missing_q14_when_other_selected")
            elif dq_present:
                # Q14 value should be one of the valid Q13 categories (excluding "Other")
                # Allow for partial matches and common variations
                valid_q14_categories = CHOICES_Q13_CATEGORIES - {"Other"}
                val_lower = val.lower().strip()
                
                # Check exact match first
                if val in valid_q14_categories:
                    pass  # Valid
                else:
                    # Check for partial matches with common variations
                    valid_match = False
                    for valid_cat in valid_q14_categories:
                        valid_cat_lower = valid_cat.lower()
                        # Check common patterns
                        if (val_lower == "trader" and "trader" in valid_cat_lower) or \
                           (val_lower == "huller" and "hulling" in valid_cat_lower) or \
                           (val_lower == "grader" and "grading" in valid_cat_lower) or \
                           (val_lower == "roaster" and "roaster" in valid_cat_lower) or \
                           (val_lower == "exporter" and "exporter" in valid_cat_lower) or \
                           (val_lower == "extractor" and "extractor" in valid_cat_lower) or \
                           (val_lower in ["shop", "coffee shop"] and "coffee shop" in valid_cat_lower) or \
                           (val_lower == "store" and "store" in valid_cat_lower) or \
                           (val_lower == "warehouse" and "warehouse" in valid_cat_lower) or \
                           (val_lower in ["mill", "wet mill"] and "mill" in valid_cat_lower):
                            valid_match = True
                            break
                    
                    if not valid_match:
                        dq_valid_choice = False
                        reasons.append("q14_not_valid_category")
    ...
    dq_pass = (dq_present and dq_valid_choice and dq_numeric_ok and 
               dq_dependency_ok and dq_contact_ok and dq_gps_ok)
...
# =============================================================================
# MAIN DATA RESHAPING FUNCTIONS
# =============================================================================

def reshape_to_long(df: pd.DataFrame, label_categories: bool) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Reshape wide format data to long format with DQC checks.
    
    Returns:
        - long_df: Long format without DQC columns
        - dqc_df: Long format with DQC columns
    """
    rows: List[Dict[str, Any]] = []
    consumed_cols: set = set()
    
    for _, w in df.iterrows():
        # Get selected categories for this response (used in DQC later)
        # selected_cats = {suf for fld, suf in CAT_FLAG_FIELDS.items() if truthy(w.get(fld))}
        
        # Q13: Business categories
        rows.extend(build_category_rows(w))
        consumed_cols.update(CAT_FLAG_FIELDS.keys())
        
        # Identity / contact questions
        identity_mappings = [
        ...
        for col, qk in identity_mappings:
        ...
        # Per-category fields
        rows.extend(build_per_category_rows(
            w, "q_{suffix}_business_name", "q15_business_name", label_categories
        ))
        rows.extend(build_per_category_rows(
            w, "q_{suffix}_bussines_address", "q16_business_address", label_categories
        ))
        rows.extend(build_per_category_rows(
            w, "q_{suffix}_max_operating_capacity", "q18_max_operating_capacity", label_categories
        ))
        rows.extend(build_per_category_rows(
            w, "q_{suffix}_max_storage", "q19_max_storage", label_categories
        ))
        
        # Mark columns as consumed
        for s in CAT_SUFFIXES:
            consumed_cols.update([
                f"q_{s}_business_name",
                f"q_{s}_bussines_address", 
                f"q_{s}_max_operating_capacity",
                f"q_{s}_max_storage"
            ])
        
        # Q21: Total processing (HS only)
        ...
        # Q20: Hullers operated
        rows.append(build_scalar_row(w, "q_huller_operated", "q20_hullers_operated"))
        consumed_cols.add("q_huller_operated")
        
        # Array fields - Type of coffee sourced
        rows.extend(build_array_rows(
            w, "q_type_coffee_sourced_json", "q22_type_of_coffee_all", 
            category="all", label_categories=False
        ))
        ...
        for s in CAT_SUFFIXES:
            ...
            if col in w.index:
                rows.extend(build_array_rows(
                    w, col, "q22_type_of_coffee", category=s, label_categories=label_categories
                ))
                consumed_cols.add(col)
        
        # Array fields - Coffee form
        rows.extend(build_array_rows(
            w, "q_coffee_form_json", "q23_coffee_form_all", 
            category="all", label_categories=False
        ))
        ...
        for s in CAT_SUFFIXES:
            ...
            if col in w.index:
                rows.extend(build_array_rows(
                    w, col, "q23_coffee_form", category=s, label_categories=label_categories
                ))
                consumed_cols.add(col)
        
        # Array fields - Receive coffee from
        rows.extend(build_array_rows(
            w, "q_recieve_coffee_from_json", "q26_receive_coffee_from_all", 
            category="all", label_categories=False
        ))
        ...
        for s in CAT_SUFFIXES:
            ...
            if col in w.index:
                rows.extend(build_array_rows(
                    w, col, "q26_receive_coffee_from", category=s, label_categories=label_categories
                ))
                consumed_cols.add(col)
        
        # Districts
        rows.append(build_scalar_row(
            w, "q_district_coffee_received", "q24_districts_received_from", category="all"
        ))
        ...
        # Annual kgs
        rows.append(build_scalar_row(
            w, "q_coffee_recieved_in_a_year_kgs", "q25_annual_kgs_received", category="all"
        ))
        ...
        # Candidate info
        candidate_fields = [
            ("q_candidate_name", "q_candidate_name"),
            ("q_candidate_phone", "q_candidate_phone"),
            ("q_candidate_id_number", "q_candidate_id_number")
        ]
        
        for c, qk in candidate_fields:
            rows.append(build_scalar_row(w, c, qk))
            consumed_cols.add(c)
        
        # Q14: Other business category (conditional on Q13)
        if "q_other_business_category" in w.index:
            rows.append(build_scalar_row(w, "q_other_business_category", "q14_other_business_category"))
            consumed_cols.add("q_other_business_category")
        
        # Generic fallback for any unconsumed columns
        for col in w.index:
...
def sort_question_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Sort columns to arrange questions sequentially (q1, q2, q3, etc.)
    while keeping meta columns at the beginning.
    """
    import re
    
    # Separate meta columns and question columns
    ...
    def extract_question_number(col_name: str) -> tuple:
        """Extract question number for sorting. Returns (question_num, col_name)"""
        # Look for patterns like q1_, q2_, q11_, q22_, etc.
        match = re.match(r'q(\d+)', col_name)
        if match:
            return (int(match.group(1)), col_name)
        else:
            # Non-question columns go to the end
            return (999999, col_name)
    
    # Sort other columns by question number, then alphabetically
    sorted_other_cols = sorted(other_cols, key=extract_question_number)
    
    # Combine meta columns + sorted question columns
    final_column_order = meta_cols + sorted_other_cols
    
    return df[final_column_order]


# =============================================================================
# PIVOT AND AGGREGATION FUNCTIONS
# =============================================================================

def agg_join_distinct(vals: Iterable[Any], joiner: str = " | ") -> Optional[str]:
    """Join distinct non-null values with separator."""
    seen, out = set(), []
...
def agg_all_true(vals: Iterable[Any]) -> Optional[bool]:
    """Check if all non-null values are true."""
    has_any = False
...
def pivot_questions_only(df: pd.DataFrame, include_dqc: bool, 
                         dqc_cols: List[str], joiner: str) -> pd.DataFrame:
    """
    Pivot long format to wide format with one column per question.
    """
    ...
    if include_dqc:
        for dqc_col in dqc_cols:
            ...
            if dqc_col == "dq_failed_reason":
                g = df.groupby(group_keys, dropna=False)[dqc_col].agg(
                    lambda s: agg_join_distinct(s.tolist(), joiner=";")
                ).reset_index()
            else:
                g = df.groupby(group_keys, dropna=False)[dqc_col].agg(
                    lambda s: agg_all_true(s.tolist())
                ).reset_index()
            
            w = g.pivot(index=meta, columns="question_key", values=dqc_col).reset_index()
            
            # Suffix DQC column names to avoid collisions
...
def generate_output_filename(base_name: str, extension: str = "csv") -> str:
    """Generate filename with timestamp in data folder."""
...
def main():
    ...
    # Output options
    ap.add_argument("--out-wide", help="Path for WIDE CSV output (if not provided, uses timestamp)")
    ap.add_argument("--out-long", help="Optional path for LONG CSV output for audit")
    
    # Processing options
    ap.add_argument("--include-dqc", action="store_true", 
                   help="Append DQC flags as extra columns (per question)")
    ap.add_argument("--dqc-cols", nargs="*", default=[
        "dq_present", "dq_valid_choice", "dq_numeric_ok", "dq_dependency_ok",
        "dq_contact_ok", "dq_gps_ok", "dq_pass", "dq_failed_reason"
    ], help="Which DQC columns to include when --include-dqc is set")
    ap.add_argument("--joiner", default=" | ", 
                   help="Joiner for multiple values per question")
    ap.add_argument("--label-categories", action="store_true",
                   help="Prefix category-specific values with codes (e.g., 'HS:', 'GF:')")
    ...
    if args.out_wide:
        ...
        if "/" in args.out_wide or "\\" in args.out_wide:
            out_wide_path = args.out_wide
        else:
            # Just filename provided, put in data folder with timestamp
            base_name = Path(args.out_wide).stem
            out_wide_path = generate_output_filename(f"vca_wide_{base_name}")
    else:
        out_wide_path = generate_output_filename("vca_wide_unified")
    
    if args.out_long:
        # Check if path includes directory
        if "/" in args.out_long or "\\" in args.out_long:
            out_long_path = args.out_long
        else:
            # Just filename provided, put in data folder with timestamp
            base_name = Path(args.out_long).stem
            out_long_path = generate_output_filename(f"vca_long_{base_name}")
    else:
        out_long_path = None
    ...
    wide = pivot_questions_only(
        dqc_df, 
        include_dqc=args.include_dqc, 
        dqc_cols=args.dqc_cols, 
        joiner=args.joiner
    )
    
    print("Sorting columns sequentially...")
    wide = sort_question_columns(wide)
    
    # Save outputs
    print(f"Saving wide format to: {out_wide_path}")
    Path(out_wide_path).parent.mkdir(parents=True, exist_ok=True)
    wide.to_csv(out_wide_path, index=False, encoding="utf-8")
    
    if out_long_path:
        print(f"Saving long format to: {out_long_path}")
        Path(out_long_path).parent.mkdir(parents=True, exist_ok=True)
        long_df.to_csv(out_long_path, index=False, encoding="utf-8")
    
    # Print summary
    print("\n" + "="*60)
    print("PROCESSING COMPLETE")
    print("="*60)
    print(f"WIDE output: {out_wide_path}")
    print(f"  - Rows: {len(wide):,}")
    print(f"  - Columns: {len(wide.columns):,}")
    
    if out_long_path:
        print(f"LONG output: {out_long_path}")
